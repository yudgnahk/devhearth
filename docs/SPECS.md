...
It is not a cache cleaner. It may identify removable generated data, but its primary purpose is structural optimization: consolidating duplicated runtimes, adopting shared dependency stores, hibernating inactive projects safely, identifying redundant container and AI assets, and preventing new duplication.
...
4. The user sees disk usage grouped by projects, ecosystems, runtimes, containers, AI assets, and recoverability.
5. The application proposes ranked optimizations.
...
The app must associate declared project requirements with installed versions before recommending consolidation.
...
- Global content-addressed stores.
- Download caches.
...
- Estimate npm/Yarn projects that could benefit from pnpm without claiming automatic compatibility.
- Estimate Python projects that could benefit from uv and APFS copy-on-write installs.
- Identify repeated runtime versions installed through different managers.
- Identify inactive reproducible project environments eligible for hibernation.
- Detect monorepo candidates conservatively; never recommend combining repositories solely from dependency similarity.

### 7.5 Containers and virtual machines
...
1. Consolidate duplicate runtime installations.
2. Adopt a shared dependency store.
3. Hibernate an inactive reproducible project.
4. Remove an obsolete worktree after Git verification.
5. Unify or relocate duplicate downloadable AI models.
6. Replace redundant per-project services with a shared local service, as guidance only.
7. Remove an unused architecture variant.
8. Pin missing runtime versions or lockfiles to make future hibernation possible.
9. Move cold but non-reproducible assets to external or managed storage.

### 7.8 Hibernation and restoration
...
- Exclusion patterns.
- Preferred runtime and package managers.
...
Absolute personal paths, repository names, asset inventory, credentials, and scan history must not be exported by default.
...
- Project, ecosystem, runtime, container, and AI-asset views.
...
- Medium: Re-download/rebuild cost, tool migration, runtime consolidation, or uncertain compatibility.
...
- Evidence-backed recommendations for runtime consolidation, dependency-store adoption, and project hibernation candidates.
...
- Automated package-manager migration.
- Automatic deletion.
...
- Reduction in duplicate runtime and dependency installation over time.
